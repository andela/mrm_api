...
class QueryRoomSchedule(BaseTestCase):
    ...
    def test_room_schdeule_with_non_existant_calendar_id(self):
        """This function tests whether an error is raised if the calandarId is 
            non existant.
        """
        query = self.client.execute(room_schedule_query_with_non_existant_calendar_id)
        self.assertNotEquals(query,room_schedule_of_non_existant_calendar_id_response)
