...
from fixtures.room_resource.room_resource_fixtures import (
    ...
    resource_mutation_empty_quantity_query, error_empty_quantity_string
)
...
class TestCreateRoomResource(BaseTestCase):
    ...
    def test_room_resource_creation_quantity_error(self):
        execute_query = self.client.execute(
            resource_mutation_empty_quantity_query,
            context_value={'session': db_session})

        expected_responese = error_empty_quantity_string
        self.assertEqual(execute_query, expected_responese)
