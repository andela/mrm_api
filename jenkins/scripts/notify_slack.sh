...
STATUS=$1 #job status
REBUILD_URL="https://jenkins.andela.com/blue/organizations/jenkins/mrm_api/detail/${GIT_BRANCH}/${BUILD_NUMBER}/pipeline"
GIT_USERNAME="$(git show -s --pretty=%an)"
get_build_report() {
  #Send a slack notification with the status of each stage
  if [[ "$STAGE_NAME" == *"test"* ]] && [[  "$STATUS" == 'success' ]]; then

    MESSAGE_TEXT="${STAGE_NAME} Phase Passed! :smiley:"
    COLOR="good"

  elif [[ "$STAGE_NAME" == *"test"* ]] && [[  "$STATUS" == 'fail' ]]; then
