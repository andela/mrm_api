...
class CancelEvent(graphene.Mutation):
    ...
    def mutate(self, info, **kwargs):
        ...
        if kwargs['number_of_participants'] < event.room.capacity:
            underbooked_notification_payload = underbooked_room_notification(
                event_name=event.event_title,
                room_name=event.room.name
            )
            create_notification(
                title=underbooked_notification_payload['title'],
                message=underbooked_notification_payload['message'],
                location_id=event.room.location_id
            )
